...
def risk_parity(bond_data, original_weights, constraints):
    ...
    print(bond_data['DTS'])
    # Parameters
    N = len(bond_data)  # Number of bonds
    ...
    # KEY ERROR: ZERO HERE
    risk_contributions = [w[i] * bond_data.iloc[i]['DTS'] for i in range(N) if i < len(bond_data)]
    ...
    model.addConstr(gp.quicksum(bond_data.iloc[i]['OAS'] * w[i] for i in range(N) if i < len(bond_data)) >= lower_bound, "MinOAS")
    model.addConstr(gp.quicksum(bond_data.iloc[i]['OAS'] * w[i] for i in range(N) if i < len(bond_data)) <= upper_bound, "MaxOAS")

    # 2. Liquidity constraint: portfolio liquidity must be at least 90% of benchmark liquidity (with relaxation)
    Liquidity = gp.quicksum(bond_data.iloc[i]['liquidity_score'] * BenchmarkWeight for i in range(N) if i < len(bond_data))
    MinLiquidity = 0.9 * Liquidity
    model.addConstr(gp.quicksum(bond_data.iloc[i]['liquidity_score'] * w[i] for i in range(N) if i < len(bond_data)) >= MinLiquidity - slack_liquidity, "MinLiquidity")

    # 3. Transaction cost constraint: within 10% of benchmark transaction cost (with relaxation)
    Benchmark_cost = gp.quicksum(bond_data.iloc[i]['transaction_cost'] * BenchmarkWeight for i in range(N) if i < len(bond_data))
    lower_t_cost = 0.9 * Benchmark_cost
    upper_t_cost = 1.1 * Benchmark_cost
    model.addConstr(gp.quicksum(bond_data.iloc[i]['transaction_cost'] for i in range(N) if i < len(bond_data)) >= lower_t_cost - slack_tcost_lower, "MintCost")
    model.addConstr(gp.quicksum(bond_data.iloc[i]['transaction_cost'] for i in range(N) if i < len(bond_data)) <= upper_t_cost + slack_tcost_upper, "MaxtCost")

    # 4. Weight sum constraint: sum of weights must be 1
    model.addConstr(gp.quicksum(w[i] for i in range(N) if i < len(bond_data)) == 1, "WeightSum")

    for constraint in constraints:
        if constraint['operator'] == '<=':
            model.addConstr(gp.quicksum(bond_data.iloc[i][constraint['column']] for i in range(N) if i < len(bond_data)) <= constraint['value'], "<=")
            ...
            model.addConstr(gp.quicksum(bond_data.iloc[i][constraint['column']] for i in range(N) if i < len(bond_data)) <= constraint['value'], "=1")
            model.addConstr(gp.quicksum(bond_data.iloc[i][constraint['column']] for i in range(N) if i < len(bond_data)) >= constraint['value'], "=2")
